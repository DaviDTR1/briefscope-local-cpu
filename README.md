...
> internet** once the model and embedding weights are downloaded. It can
> optionally be pointed at a cloud provider from the Settings UI. For other
> backends see the [other variants](#other-variants).
...
- **LLM**: a bundled `ollama/ollama` container (`briefscope_ollama_cpu`),
...
Key REST routes (all under the plugin root path `/api/briefscope_local_cpu`):
...
- Enough RAM for the chosen Ollama model (the default `llama3.2` needs ~2 GB).
...
BriefScope is a self-contained FastAPI app plus bundled Ollama and ChromaDB
containers. It does not need the kernel, Traefik or any other plugin to run —
only Docker. No API keys are required.

```bash
# Build and start everything (app + Ollama + ChromaDB)
...
```bash
docker exec briefscope_ollama_cpu ollama pull llama3.2
```

`docker-compose.standalone.yml` is **self-contained**: it defines every service
(app + Ollama + ChromaDB), its own private network and volumes, publishes the
app on host port `8080`, and blanks `ROOT_PATH` so the UI, REST API and
...
3. Find **BriefScope LOCAL CPU** in the catalog and install it.
...
at `/api/briefscope_local_cpu`; the base `docker-compose.yml` is used as-is (no
standalone override).
...
|---|---|---|
| `ROOT_PATH` | `/api/briefscope_local_cpu` | FastAPI `root_path` / kernel path prefix. Blanked to `""` in `docker-compose.standalone.yml` so everything serves at the host root. |
| `LLM_MODE` | `local` | Selects the local (Ollama) path. Leave as-is. |
| `OLLAMA_HOST` | `http://briefscope_ollama_cpu:11434` | Bundled Ollama container. Leave as-is. |
