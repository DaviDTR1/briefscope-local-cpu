...
- **Embeddings now run on Ollama instead of sentence-transformers.** Document
  search embeddings are computed by the bundled Ollama server (`/api/embed`) and
  passed to ChromaDB explicitly, dropping the `sentence-transformers`/torch
  dependency to lower resource usage and image size. Default model is
  `nomic-embed-text` (`mxbai-embed-large`, `bge-m3` and `qwen3-embedding` also
  available); the model is pulled into Ollama automatically on first use.
  Switching the embedding model invalidates previously indexed documents —
  re-upload them so they are re-embedded.
...
### Added
- **Web search for the local and cloud agents.** A per-project globe toggle next to the
